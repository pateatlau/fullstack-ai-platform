...
- **Voice interfaces (V2 Epic 4)** — When `VOICE_ENABLED=true`, authenticated users can open a WebSocket at `GET /api/voice/ws?session_id={chat_session_id}` (Bearer JWT). JSON-framed messages carry base64 PCM16 24 kHz mono audio (`audio_in` / `audio_out`) and text events (`transcript_*`, `assistant_text_delta`, `tool_*`, `turn_complete`). Final transcripts invoke `UnifiedChatService.stream_execute()` — same RAG/agent/MCP path as SSE chat. Default is **off**; flag-off leaves text SSE unchanged. See `app/routers/voice.py`, `app/ai/voice/`, and `VOICE_*` settings in `.env.example`.
