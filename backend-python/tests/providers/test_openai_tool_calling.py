"""OpenAI provider tool-calling adapter tests."""
...
async def test_openai_complete_chat_with_tools_parses_tool_calls(
    ...
    assert completion.tool_calls == [
        ProviderToolCall(
            id="call_abc", name="web_search", arguments={"query": "weather"}
        )
    ]
    assert completion.content is None
    assert completion.finish_reason == "tool_calls"
    assert mock_create.await_args is not None
    create_kwargs = dict(mock_create.await_args.kwargs)
    assert create_kwargs["tools"]


async def test_openai_complete_chat_with_tools_handles_direct_answer(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    provider = OpenAIProvider(api_key="test-key")
